...
// Componente CalculadorSueldo simplificado (placeholder)
const CalculadorSueldo = () => {
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-md p-6">
      <h3 className="text-xl font-semibold mb-4">Calculador de Sueldo Sugerido</h3>
      <p className="text-gray-600">Funcionalidad de cálculo de sueldo aquí...</p>
    </div>
  );
};
...
const CostCalculator = () => {
  ...
  const handleVariableCostChange = (index, field, value) => {
  ...
  // Handlers para gastos operacionales
  ...
  const handleOperationalCostChange = (index, field, value) => {
    ...
    newCosts[index][field] = field === 'monto' ? (parseFloat(value) || 0) : value;
  ...
  // Cálculos corregidos
  const totalFixedCosts = fixedCosts.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
  const totalVariableCosts = variableCosts.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
  const totalOperationalCosts = operationalCosts.reduce((acc, item) => acc + (parseFloat(item.monto) || 0), 0);
  const totalMaterials = materials.reduce((sum, m) => sum + ((parseFloat(m.quantity) || 0) * (parseFloat(m.price) || 0)), 0);
  const totalHours = (parseFloat(hours) || 0) * (parseFloat(rate) || 0);
  const totalExpenses = expenses.reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0);
  const totalCost = totalFixedCosts + totalVariableCosts + totalOperationalCosts + totalMaterials + totalHours + totalExpenses;
  const unitCost = productCount > 0 ? totalCost / productCount : 0;
  const totalFijosVariablesOperativos = totalFixedCosts + totalVariableCosts + totalOperationalCosts;
  // Totales consolidados para el resumen
  const fixedCostsTotal = totalFixedCosts;
  const variableCostsTotal = totalVariableCosts;
  const operationalCostsTotal = totalOperationalCosts;
  ...
  const exportToExcel = () => {
    const wsData = [
      ...
      ['Descripción', 'Monto'],
      ...operationalCosts.map(item => [item.descripcion, item.monto]),
      ['TOTAL GASTOS OPERACIONALES', totalOperationalCosts],
      [],
      ['Materiales'],
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-8">
            ...
            <div key={index} className="flex items-center gap-2 mb-2">
              ...
              <div className="flex items-center border rounded px-2">
                ...
                <input
                  type="number"
                  value={item.monto || ''}
                  onChange={(e) => handleOperationalCostChange(index, 'monto', e.target.value)}
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-md p-6 mb-6">           
        <div className="space-y-4">
          <h3 className="text-xl font-semibold mb-4">Materiales</h3>
          {materials.map((m, i) => (
      ...
      <div className="text-center mt-8 p-6 bg-gray-50 rounded-lg">
        <p className="text-lg font-medium">
          Total de gastos fijos, variables y operativos:{" "}
          <span className="text-blue-600 font-bold">
            ${(fixedCostsTotal + variableCostsTotal + operationalCostsTotal).toFixed(2)}
          </span>
        </p>
        
        <p className="text-lg font-medium mt-2">
          Costo de lo que vendes:{" "}
          <span className="text-purple-600 font-bold">
            ${totalMaterials.toFixed(2)}
          </span>
        </p>
        <p className="text-lg font-medium">
            Costo unitario promedio: <span className="font-bold text-xl text-orange-600">${unitCost.toFixed(2)}</span>
          </p>
